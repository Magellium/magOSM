...
Docker gère le load balancing entre les différentes instances de geoserver déployées. Il n'est pas possible d'accéder à l'interface d'administration de geoserver lorsque plusieurs containers du service `geoserver` sont lancés en même temps (les requêtes d'authentification et d'accès aux données ne sont pas
